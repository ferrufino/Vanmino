...
class Hike {
    ...
    func initVariables(trailId: String, hikeDetails: AnyObject){
        ...
        imgURL = hikeDetails["imgURL"] as? String ?? ""
        img = nil
    ...
    func copyFromCoreData(_ hike: Trail){
        id = hike.trailId ?? ""
        name = hike.name
        distance = hike.distance
        time = hike.time
        elevation = hike.elevationGain
        coor.coordinatesForTrail = hike.coordinates ?? []
        region = hike.region
        img = UIImage(data: hike.staticImage!)
        publicTrail = false
